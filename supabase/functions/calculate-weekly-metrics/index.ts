...
Deno.serve(async (req) => {
  ...
  const syncLog = await startSyncLog('calculate-weekly-metrics');
  ...
  try {
    ...
    // Parse request body for optional weeks parameter
    let weeksToCalculate = 4;
    try {
      const body = await req.json();
      if (body.weeks) weeksToCalculate = body.weeks;
    } catch {
      // Use default
    }

    console.log(`Calculating weekly metrics for last ${weeksToCalculate} weeks...`);
    ...
    // Get locked weeks to skip
    const { data: lockedWeeks } = await supabase
      .from('weekly_metrics')
      .select('week_start')
      .eq('is_locked', true);
    
    const lockedWeekSet = new Set(lockedWeeks?.map(w => w.week_start) || []);
    console.log(`Found ${lockedWeekSet.size} locked weeks that will be skipped`);
    ...
    for (let i = 0; i < weeksToCalculate; i++) {
      ...
      // Skip locked weeks
      if (lockedWeekSet.has(startStr)) {
        console.log(`Skipping locked week: ${startStr}`);
        continue;
      }
      ...
      console.log(`FTDs by channel for week ${startStr}:`, JSON.stringify(ftdsByChannel));
      ...
      // 4. Get Mixpanel events (signups, FTDs, STDs)
      const { count: signupCount } = await supabase
        .from('mixpanel_events')
        .select('*', { count: 'exact', head: true })
        .eq('event_name', 'signup_completed')
        .gte('event_time', startTimestamp)
        .lte('event_time', endTimestamp);

      // Use RPC for unique FTD count (eliminates duplicate events)
      const { data: ftdCountData } = await supabase.rpc('get_unique_ftd_count', {
      ...
      const totalFTDs = ftdCountData || 0;
      const totalSTDs = stdCount || 0;
      ...
      // 8c. HVP Count (users who deposit £250+ in first 7 days after FTD)
      const { data: hvpData, error: hvpError } = await supabase.rpc('get_report_hvp_count', {
        start_date: startStr,
        end_date: endStr,
      });

      if (hvpError) {
        console.error(`Error fetching HVP count for ${startStr}:`, hvpError);
      }
      console.log(`HVP data for ${startStr}:`, JSON.stringify(hvpData));

      const totalHVPs = hvpData?.[0]?.value || 0;
      const costPerHVP = totalHVPs > 0 ? totalSpend / totalHVPs : 0;
      ...
      // 12. Average rating (weighted) - includes App Store, Google Play, Trustpilot, and Typeform
      ...
      const { data: typeformSurveys } = await supabase
        .from('typeform_surveys')
        .select('rating')
        .gte('submitted_at', startTimestamp)
        .lte('submitted_at', endTimestamp);
      ...
      const trustpilotAvg = trustpilotReviews?.length 
        ...
        : 0;
      const typeformAvg = typeformSurveys?.length 
        ? typeformSurveys.reduce((sum, r) => sum + r.rating, 0) / typeformSurveys.length 
        : 0;
      ...
      if (typeformSurveys?.length) {
        weightedSum += typeformAvg * weights.typeform;
        weightTotal += weights.typeform;
      }
    ...
    // Upsert all weekly metrics
    console.log(`Upserting ${results.length} weekly metrics...`);
    
    const { error: upsertError } = await supabase
      .from('weekly_metrics')
      .upsert(results, { onConflict: 'week_start' });
    ...
    console.log('Weekly metrics calculation complete');
    ...
    return new Response(
      JSON.stringify({ 
        ...
        weeks: results.map(r => r.week_start),
      }),
